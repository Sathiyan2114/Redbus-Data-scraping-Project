...
st.set_page_config(
    page_title="Red Bus",
    page_icon=r"C:\Users\vijay\Downloads\redbus_logo.jpg"
)
...
def fetch_data(query):
    try:
        ...
        with mydb.cursor(buffered=True) as cursor:
            ...
            data = pd.DataFrame(result, columns=columns)  # Convert to a DataFrame
        return data
    except mysql.connector.Error as e:
        st.error(f"Error fetching data: {e}")  # Show an error in Streamlit if there's a problem
...
try:
    ...
    if initial_data.empty:  # Check if the data is empty
